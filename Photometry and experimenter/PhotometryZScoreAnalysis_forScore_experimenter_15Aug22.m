...
BLOCKPATH = fullfile(DATAPATH,'FP_Experimeter_Example-220602-162235'); %Data file have to be in the MATLAB\TDTMatlabSDK\Examples\ directory
...
REF_EPOC = 'Score'; % event store name. This was manually scored using TDT OpenScope software.
VALUE = 1; % score event code we are interested in; 1 is experimenter delivered stimulation
...
AUC=[]; % pre, post
AUC(1,1)=trapz(mean(zall(:,ts2(1,:) > -2 & ts2(1,:) < 0)));
